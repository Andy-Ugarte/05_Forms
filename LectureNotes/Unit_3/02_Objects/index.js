...
console.log(typeof theSimpsons.currently_running);
...
simpsonsCharacters.showAge = 'Flanders';
...
const output = []; // ? can be either "let" or "const"
let concat_output = ''; // ! need to be a "let" variable

for (let keyName in simpsonsFamily) {
  let result = `${keyName}: ${simpsonsFamily[keyName]}`;
  output.push(result);
  concat_output += `, ${result}`; //? concat_output = concat_output + `, ${result}`
}
