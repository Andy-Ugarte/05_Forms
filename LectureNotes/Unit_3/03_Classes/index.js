...
class Cookie {
  ...
  */
  ...
  const firstCookie = new Cookie();
  console.log(firstCookie);
  firstCookie.shape = "Triangle";
  console.log(firstCookie);
  
  const secondCookie = new Cookie('Square', 'Blue Maple', 12.45);
  ...
  */
  ...
  class Motorcycle extends Car {
    constructor(wheels, engine, passengers, requiresHelmet, canWheelie) {
      // if I want to use "super", it must be the first item in my constructor, before I use "this" on any other property
      super(wheels, engine, passengers); // act like the constructor in the parent class (Car)
      // super(); // MUST call super() constructor before using "this", even if the super is empty, it must be called first.
